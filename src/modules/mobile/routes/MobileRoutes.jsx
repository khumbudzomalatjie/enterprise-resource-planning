...
function IncidentReportPage() {
  ...
  const [formData, setFormData] = useState({ job_id: '', incident_type: 'other', description: '', severity: 'medium' })
  ...
  const handleSubmit = async () => {
    if (!formData.description.trim()) { toast.error('Please describe the incident'); return }
    setSubmitting(true)
    try {
      let photoUrl = null
      if (photoFile) {
    ...
    <div className="min-h-screen bg-slate-50 font-['Inter'] pb-20">
      ...
      <div className="px-4 -mt-4 space-y-4 pt-4">
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          <select value={formData.job_id} onChange={e => setFormData({...formData, job_id: e.target.value})} className="w-full p-3 rounded-xl bg-slate-50 border border-slate-200 text-sm">
          ...
          <div className="flex gap-2">
            {[{ value: 'low', label: 'Low', color: 'bg-blue-500' },{ value: 'medium', label: 'Medium', color: 'bg-amber-500' },{ value: 'high', label: 'High', color: 'bg-orange-500' },{ value: 'critical', label: 'Critical', color: 'bg-red-500' }].map(sev => (
              <button key={sev.value} onClick={() => setFormData({...formData, severity: sev.value})}
                className={`flex-1 py-2.5 rounded-xl text-sm font-medium ${formData.severity === sev.value ? `${sev.color} text-white` : 'bg-slate-100 text-slate-600'}`}>{sev.label}</button>
            ))}
          </div>
        </div>
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          <label className="text-sm font-semibold text-slate-500 mb-2 block">Description *</label>
          <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} rows={4} placeholder="Describe what happened..." className="w-full p-3 rounded-xl bg-slate-50 border border-slate-200 text-sm resize-none" />
        </div>
        <div className="bg-white rounded-2xl p-4 shadow-sm">
        ...
        <button onClick={handleSubmit} disabled={submitting} className="w-full bg-red-500 text-white rounded-2xl p-4 font-bold text-lg hover:bg-red-600 disabled:opacity-50 active:scale-95 transition-all flex items-center justify-center gap-2 shadow-lg">
          <Send className="w-5 h-5" />{submitting ? 'Submitting...' : 'Submit Incident Report'}
        </button>
      </div>
      <BottomNav />
    </div>
