...
export default function MobileHome() {
  ...
  // Load ALL jobs assigned to this cleaner (excluding held and completed)
  const loadAllJobs = async () => {
    ...
    try {
      ...
      // Get ALL jobs EXCEPT 'completed' and 'on_hold' (held)
      let query = supabase
        .from('jobs')
  ...
  // COMPLETE JOB - Auto-moves to finance for invoicing
  const handleCompleteJob = async (jobId) => {
    if (!window.confirm('Mark as completed? This will send the job for invoicing.')) return
    setUpdatingJob(jobId)
    try {
      const { error } = await supabase
        .from('jobs')
        ...
        .eq('id', jobId)
      if (error) throw error
      toast.success('Job completed! Moving to finance for invoicing ✅')
      loadData()
  ...
  const handlePauseJob = async (jobId) => {
    ...
    try {
      const { error } = await supabase
        ...
        .eq('id', jobId)
      if (error) throw error
      toast.success('Job paused - will be removed from your list')
      loadData()
  ...
  // Separate today's jobs for the top section
  const todayJobs = filteredJobs.filter(j => j.scheduled_date === todayStr)
        ...
        <div className="px-5 -mt-3">
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              { icon: List, label: 'All Open', value: allJobs.length, color: 'from-purple-400 to-violet-500' },
            ].map((s, i) => (
        ...
        <div className="px-5 mt-5 mb-4">
          ...
          {loadingAllJobs ? (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex-1 cursor-pointer" onClick={() => navigate(`/mobile/jobs/${job.id}`)}>
                        ...
                        <p className="text-xs text-slate-400">
                          {job.job_number} · {job.clients?.company_name || 'Client'}
                        </p>
                      </div>
                    ...
                    {job.clients?.phone && (
                      <a href={`tel:${job.clients.phone}`} 
                        className="mt-2 text-xs text-emerald-600 flex items-center gap-1 bg-emerald-50 rounded-lg px-2 py-1 w-fit hover:bg-emerald-100 transition-colors">
                        📞 Call {job.clients.company_name?.split(' ')[0]}
                      </a>
                    )}
                  </motion.div>
                )
            ...
            <div className="text-center py-10 bg-white/10 backdrop-blur rounded-2xl">
              ...
              <p className="text-white font-semibold">All jobs completed! 🎉</p>
              <p className="text-white/60 text-xs mt-1">
                {jobSearch ? 'No jobs match your search' : selectedDate !== 'all' ? 'No jobs for this date' : 'Pull down to refresh'}
              </p>
            </div>
          )}
        </div>
